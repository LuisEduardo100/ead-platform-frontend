...
const courseService = {
  ...
  getSearch: async (name: string, year: string | null) => {
    const query = name
      ? `/courses/search?name=${name}&serie=${year}`
      : `/courses/search?name=&serie=${year}`;
    const res = await api.get(query).catch((error) => {
      return "Erro aqui" + error.message;
    });
    return res;
  },
  getSearchGeneral: async (name: string) => {
    const res = await api
      .get(`/courses/gsearch?name=${name}`)
      .catch((error) => {
        return "Erro aqui" + error.message;
      });
    return res;
